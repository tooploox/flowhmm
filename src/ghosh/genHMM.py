    ...
    def report(self, logprob):
        ...
        if self.verbose:
            ...
            message = self._template.format(
                iter=self.iter + 1, log_prob=logprob, delta=self.delta_rel
            )
...
class GenHMM(torch.nn.Module):
    def __init__(
        ...
        self.init_gen(
            H=net_H, D=net_D, nchain=net_nchain, mask_type=mask_type, p_drop=p_drop
        )
    ...
    def init_gen(self, H, D, nchain, mask_type="cross", p_drop=0.25):
        ...
        prior = distributions.MultivariateNormal(
            torch.zeros(D).to(self.device), torch.eye(D).to(self.device)
        )
    ...
    def _getllh(self, networks, batch):
        ...
        for s in range(self.n_states):
            ...
            loglh_sk = [
                networks[s, k].log_prob(x, x_mask) / x.size(2)
                for k in range(self.n_prob_components)
            ]
