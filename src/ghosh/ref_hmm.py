        ...
        for iter in range(self.n_iter):
            ...
            for i, j in iter_from_X_lengths(X, lengths):
                ...
                self._accumulate_sufficient_statistics(
                    stats, X[i:j], framelogprob, posteriors, fwdlattice, bwdlattice
                )
...
class GMM_HMM(hmm.GMMHMM):
    ...
    def _accumulate_sufficient_statistics(
        self, stats, X, framelogprob, post_comp, fwdlattice, bwdlattice
    ):
    ...
    def _do_mstep(self, stats, X):
        ...
        post_mix_sum = np.concatenate(
            [x[..., np.newaxis] for x in stats["post_mix_sum"]], axis=-1
        )
        post_sum = np.concatenate(
            [x[..., np.newaxis] for x in stats["post_sum"]], axis=-1
        )
        ...
        new_means_numer = (
            np.einsum("ijk,il->jkl", post_comp_mix, X) + lambdas[:, :, np.newaxis] * mus
        )
        ...
        """
        new_cov_denom = post_mix_sum.sum(-1)[:, :, np.newaxis] + 1 + 2 * (alphas + 1)
