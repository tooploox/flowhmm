...
def batch_data(n_components):
    ...
    data = np.array(
        [np.concatenate((xx, np.zeros((length - xx.shape[0] + 1, d)))) for xx in x]
    )
...
torch_logprob, torch_fwdlattice = _forward(
    int(data.shape[1]),
    int(n_components),
...
torch_log_xi_sum = _compute_log_xi_sum(
    int(data.shape[1]),
    int(n_components),
    torch_fwdlattice,
    torch.from_numpy(np.log(transmat)),
