...
class ActNorm(nn.Module):
    ...
    def __init__(self, num_channels, scale = 1., logscale_factor=3., batch_variance=False):
        ...
        self.register_parameter('bias', nn.Parameter(torch.zeros(1, self.num_channels, 1)))
        self.register_parameter('logs', nn.Parameter(torch.zeros(1, self.num_channels, 1)))
    ...
    def forward(self, x, logdet=None):
        ...
        assert x.shape[1] == self.num_channels, \
            'Input shape should be N_B x N_C x N_S , however channels are {} instead of {}'.format(x.shape[2], self.num_channels)
    ...
    def reverse(self, z, logdet=None):
        ...
        assert z.shape[1] == self.num_channels, \
            'Input shape should be N_B x N_C x N_S, however channels are {} instead of {}'.format(z.shape[2], self.num_channels)
    ...
    def reverse(self, x, logdet=None):
        ...
        if not self.lu_decomposition:
            ...
            weight = torch.matmul((self.weight_U + self.s).inverse().float(), \
                     torch.matmul(self.weight_L.inverse().float(), self.weight_P.inverse().float()))
            weight =  weight.view(*weight.shape, 1)
            z = F.conv1d(x, weight)
            if logdet is not None:
                logdet = logdet - dlogdet
...
class Permutation1d(nn.Module):
    ...
    def __init__(self, num_channels, shuffle=False, switch_flag=True):
        ...
        if shuffle == False and switch_flag == True:
            ...
            self.indices_inverse = torch.from_numpy(np.flip(self.indices_inverse).copy())
...
class Split1D(nn.Module):
    ...
    def forward(self, x, logdet=0., eps_std=None):
        """
    ...
    def reverse(self, x, logdet=0., eps_std=None):
        """
...
class Squeeze1D(nn.Module):
    ...
    def squeeze(x, factor=2):
        ...
        """
        assert factor >= 1 # Check if the factor is at least more than one, otherwise invalid
        if factor == 1: # If the factor is 1, then there is no question of squeezing, identical tensor returned
            return x 
        ...
        x = x.view(-1, nc, ns // factor, factor) # Reshaping the number of spatial dimensions by adjusting the batch size
        x = x.permute(0, 1, 3, 2).contiguous() # Permute the dimensions so that output shape is similar to input shape
        x = x.view(-1, nc * factor, ns // factor) # So that shape again becomes (Nb x Nc x Ns)
    ...
    def unsqueeze(z, factor=2):
        ...
        z = z.view(-1, nc // factor, factor, ns) # Reducing the channel dimension by adjusting the batch size
        z = z.permute(0, 1, 3, 2).contiguous() # Permuting the dimensions so that shape is similar to original input
        z = z.view(-1, nc // factor, ns * factor) # Reshaping the tensor with proper batch size
        return z 
    ...
    def forward(self, x, logdet=None):
        ...
        """
        output = self.squeeze(x, self.factor) # Returns the output of a Squeeze operation
