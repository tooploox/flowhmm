    ...
    def report(self, logprob):
        ...
        if self.verbose:
            ...
            message = self._template.format(
                iter=self.iter + 1, logprob=logprob, delta=self.delta_rel
            )
        ...
        self.history.append(
            logprob
        )  # History contains logprob of the data for last 2 iterations
...
class GenHMM(torch.nn.Module):
    ...
    def __init__(
        ...
        transmat_type="random upper triangular",
        permutation="invconv",
        coupling="affine",
        actnorm_scale=1.0,
        ...
        self.n_prob_components = (
            n_prob_components  # No. of prob_components for every state of the HMM
        )
        ...
        self.init_transmat(
            transmat_type
        )  # Initialise the transition probability matrix A
        self.init_startprob(
            startprob_type
        )  # Initialise the initial starting probabilities of the HMM
        ...
        )  # Initialise the HMM probabbility matrix
    ...
    def init_startprob(self, startprob_type="random"):
        ...
        """
        if (
            "random" in startprob_type
        ):  # Randomly initialises the elements of the start_prob vector and subsequently normalizes the vector
            ...
            init = torch.zeros(
                self.n_states
            )  # Initialise one of the elements of the start_prob vector as 1 and rest as 0s
            init[0] = 1
            self.startprob_ = init

        elif "uniform" in startprob_type:
            self.startprob_ = torch.ones(
                self.n_states
            )  # Uniform initialisation of the start_prob vector

        normalize(
            self.startprob_, axis=0
        )  # So that the sum of the probabilities of the states sums to 1
        return self
    ...
    def init_transmat(self, transmat_type="random upper triangular"):
        ...
        if "random" in transmat_type:
            ...
            ).abs()  # Randomly initialises the transition prob. matrix
        elif "uniform" in transmat_type:
            init = 1 / self.n_states
            self.transmat_ = (
                torch.ones(self.n_states, self.n_states) * init
            )  # Uniform initialisation of the transition prob. matrix
        elif "triangular" in transmat_type:
            # use upper tra matrix
            self.transmat_ = (
                torch.randn(self.n_states, self.n_states)
            ).abs()  # Initialises the transition prob. matrix as a random upper triangular matrix
            ...
            self.transmat_ = (
                torch.ones(self.n_states, self.n_states)
                + torch.randn(self.n_states, self.n_states) * 0.01
            )  # Initialises the transition prob. matrix as an ergodic matrix

        normalize(
            self.transmat_, axis=1
        )  # Normalizes column-wise so that the transitions of the HMM are valid
        return self
    ...
    def init_gen(
        ...
        prior = distributions.MultivariateNormal(
            torch.zeros(n_D).to(self.device), torch.eye(n_D).to(self.device)
        )
        ...
        print(
            "The total number of params: {} and the number of trainable params:{}".format(
        ...
        self.old_networks = np.array(self.old_networks).reshape(
            self.n_states, self.n_prob_components
        )
        ...
        self.optimizer = torch.optim.Adam(
            sum(
                [
                    ...
                    for flow in self.networks.reshape(-1).tolist()
                ],
                [],
            ),
            lr=self.lr,
    ...
    def _update_old_networks(self):
        ...
        for i in range(self.n_states):
            for j in range(self.n_prob_components):
                self.old_networks[i, j].load_state_dict(
                    self.networks[i, j].state_dict()
                )
        return self
    ...
    def pushto(self, device):
        ...
        for s in range(self.n_states):
            for k in range(self.n_prob_components):
                ...
                self.networks[s, k].prior = type(p)(
                    p.loc.to(device), p.covariance_matrix.to(device)
                )

                # push the old networks to device
                self.old_networks[s, k].to(device)
                p = self.old_networks[s, k].prior
                self.old_networks[s, k].prior = type(p)(
                    p.loc.to(device), p.covariance_matrix.to(device)
                )
    ...
    def _initialize_sufficient_statistics(self):
        ...
        stats = {
            ...
            "mixture": torch.zeros(self.n_states, self.n_prob_components).to(
                self.device
            ),
    ...
    def _accumulate_sufficient_statistics(
        self, framelogprob, mask, posteriors, logprob, fwdlattice, bwdlattice, loglh_sk
    ):
        ...
        log_xi_sum = _compute_log_xi_sum(
            n_samples,
            n_components,
            fwdlattice,
            ...
            bwdlattice,
            framelogprob,
            torch.ones(batch_size, n_components, n_components, device=self.device)
            * float("-inf"),
            logprob,
            mask,
        )
        ...
        gamma_ = (
            self.pi.reshape(1, 1, self.n_states, self.n_prob_components)
    ...
    def _do_forward_pass(self, framelogprob, mask):
        ...
        return _forward(
            n_samples, n_components, log_startprob, log_transmat, framelogprob, mask
        )
    ...
    def _getllh(self, networks, batch):
        ...
        local_loglh_sk = torch.zeros(
            (batch_size, n_samples, self.n_states, self.n_prob_components)
        ).to(self.device)
        ...
        for s in range(self.n_states):
            ...
            loglh_sk = [
                networks[s, k].log_prob(x, self.in_channels, x_mask) / x.size(2)
                for k in range(self.n_prob_components)
            ]
            ll = torch.stack(loglh_sk).permute(1, 2, 0)
            local_loglh_sk[:, :, s, :] = ll
            llh[:, :, s] = torch.logsumexp(
                (self.logPIk_s[s].reshape(1, 1, self.n_prob_components) + ll).detach(),
                dim=2,
            )
    ...
    def forward(self, batch, testing=False):
        ...
        with torch.no_grad():
            ...
            log_num = old_loglh_sk.detach() + self.logPIk_s.reshape(
                1, self.n_states, self.n_prob_components
            )
            # log_num = brackets.detach()
            log_denom = torch.logsumexp(log_num, dim=3)

            logpk_sX = log_num - log_denom.reshape(
                batch_size, n_samples, self.n_states, 1
            )
        ...
        brackets[x_mask] = self.loglh_sk[x_mask] + self.logPIk_s.reshape(
            1, self.n_states, self.n_prob_components
        )
        ...
        loss = -(
            post[x_mask] * (torch.exp(logpk_sX) * brackets)[x_mask].sum(2)
    ...
    def fit(self, traindata):
        ...
        ada_lr = step_learning_rate_decay(
            init_lr=self.lr,
        ...
        for i in range(self.em_skip):
            ...
            print(
                "epoch:{}\tclass:{}\tStep:{}\tb:{}\tLoss:{}\tNLL:{}".format(
                    self.iepoch,
                    self.iclass,
                    i,
                    b,
                    total_loss / (b + 1),
                    -total_logprob / n_sequences,
                ),
                file=sys.stdout,
            )
        ...
        self.startprob_ = torch.where(
            self.startprob_ == 0.0, self.startprob_, startprob_
        )
        ...
        self.transmat_ = torch.where(self.transmat_ == 0.0, self.transmat_, transmat_)
        ...
        print(
            "epoch:{}\tclass:{}\tLatest NLL:\t{}".format(
                self.iepoch, self.iclass, self.latestNLL
            ),
            file=sys.stdout,
        )
