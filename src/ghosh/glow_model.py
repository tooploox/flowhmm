...
class FlowStep(nn.Module):
    ...
    def __init__(
        ...
        assert (
            permutation in self.flow_permutation_list
        ), "Unsupported flow permutation: {}".format(permutation)
        assert (
            coupling in self.flow_coupling_list
        ), "Unsupported flow coupling: {}".format(coupling)
        ...
        if permutation == "invconv":
            self.invconv = Invertible1x1Conv(
                num_channels=in_channels, lu_decomposition=lu_decomposition
            )
        elif permutation == "reverse":
            self.permrev = Permutation1d(
                num_channels=in_channels, shuffle=False, switch_flag=switch_flag
            )
        else:
            self.shuffle = Permutation1d(
                num_channels=in_channels, shuffle=True, switch_flag=switch_flag
            )

        # flow coupling layer
        if coupling == "additive":
            self.NN = NN(
                in_channels // 2, hidden_channels, in_channels // 2, weightnorm_flag
            )
        else:
            self.NN = NN(
                in_channels // 2, hidden_channels, in_channels, weightnorm_flag
            )
    ...
    def normal_flow(self, x, logdet=None):
        ...
        if self.coupling == "additive":
            ...
            scale = torch.sigmoid(self.rescale(scale) + 2.0)
        ...
        z = cat_channel(
            z1, z2, split_dim=1
        )  # rejoin the two parts of the channel (along dim 1) and formulate the output tensor
        return z, logdet
    ...
    def reverse_flow(self, z, logdet=None):
        ...
        x = cat_channel(
            x1, x2, split_dim=1
        )  # Rejoins the output tensor along the dimension 1
    ...
    def forward(self, x, logdet=None):
        ...
        return self.normal_flow(
            x, logdet
        )  # Perform one step of flow in the normalizing direction
    ...
    def reverse(self, z, logdet=None):
        ...
        return self.reverse_flow(
            z, logdet
        )  # Perform one step of flow in the generative direction
...
class FlowModel_GLOW(nn.Module):
    def __init__(
        ...
        self.nets = (
            nn.ModuleList()
        )  # Initialise the network with a modulelist of layers
        n_channels = (
            self.in_channels
        )  # Defines the mini-batch size, number of channels, no. of samples in each mini-batch
        ...
        for l in range(L):
            ...
            if l < L - 1:
                self.nets.append(
                    Split1D(num_channels=n_channels, factor=self.sq_factor)
                )
                n_channels = n_channels // self.sq_factor
                # self.output_shapes.append([-1, n_channels, n_samples])

    def f(self, z, logdet=0.0):
        """
    ...
    def g(self, z, eps_std=None):
        ...
        for layer in reversed(self.nets):
            if isinstance(layer, Split1D):
                z, _ = layer.reverse(z, logdet=0.0, eps_std=eps_std)
            else:
                z, _ = layer.reverse(z, logdet=0.0)
        return z
    ...
    def log_prob(self, x, in_channels, x_mask=None):
        ...
        if type(x_mask) == type(
            None
        ):  # x_mask decides if there are appended zeros for the samples which are not to be processed
            return px
        else:
            px[1 - x_mask] = 0
            return px
...
def main():
    ...
    n_input_samples = 200
    X = datasets.make_moons(n_samples=n_input_samples, noise=0.05)[0].astype(np.float32)
    ...
    optimizer = torch.optim.Adam(
        [p for p in flow.parameters() if p.requires_grad == True], lr=1e-4
    )

    N_iter = 4000
    savedir = (
        "./NormFlowModel/GLOW_Model/figures_final_nsamples_LU_WN"
        + str(n_input_samples)
        + "/"
    )
    # Training the model
    ...
    with detect_anomaly():
        ...
        for t in range(N_iter):
            ...
            if t % 500 == 0 and t != 0:
                print(
                    "Iteration {}, loss : {}, Elapsed Time:{}".format(
                        t, loss, abs((start_time - datetime.now()).total_seconds())
                    )
                )
